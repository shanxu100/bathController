...
public class CustomFrame {
    ...
    public static final byte SOF_FRAME = 0x5C;
    public static final byte EOF_FRAME = 0x2F;
    ...
    /**
     * 将 Data部分 的byte[]封装成 帧
     *
     * @param data
     * @return
     */
    public static byte[] encapsulateFrame(byte[] data) {

        byte header = MsgConstant.FRAME_SOF;
        byte[] length = ByteUtil.toByteArray(data.length);
        byte[] crc16 = CheckCRC.Make_CRC(length, data);
        byte tail = MsgConstant.FRAME_EOF;

        CustomFrame customFrame = new CustomFrame(header, length, data, crc16, tail);
        return customFrame.toBytes();
    }
