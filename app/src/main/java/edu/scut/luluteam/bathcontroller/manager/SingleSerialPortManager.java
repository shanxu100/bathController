...
public class SingleSerialPortManager implements OnOpenSerialPortListener, OnSerialPortDataListener {
    ...
    public void sendBytes(byte[] bytes) {
        if (bytes != null) {
            ...
            Log.i(TAG, "向串口发数据：" + ByteUtil.byte2hex(bytes));
        }
    }
...
//        final byte[] finalBytes = bytes;
        edu.scut.luluteam.bathcontroller.manager.frame.guan.FrameManager.put(bytes);
    }
    ...
    public void onDataSent(byte[] bytes) {
        Log.i(TAG, "通过串口发送 [ byte[] ]: " + Arrays.toString(bytes));
        final byte[] finalBytes = bytes;
